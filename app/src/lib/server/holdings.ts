...
/**
 * xStocks, Ondo and PreStocks tokens use Token-2022. xStocks and Ondo pass dividends through by raising a Token-2022 UI
...
/**
 * Balances `owner` holds on mainnet of the given Token-2022 mints, in UI units
 * (scaled by each issuer's multiplier). Read-only. Mints held at zero are omitted.
 */
export async function token2022Balances(owner: PublicKey, mints: ReadonlySet<string>): Promise<Map<string, number>> {
  ...
  for (const { account } of value) {
    ...
    if (!mints.has(info.mint)) continue;
  ...
  const accounts = await connection.getMultipleParsedAccounts(held.map(([mint]) => new PublicKey(mint)));
  const now = Math.floor(Date.now() / 1000);
  return new Map(
    held.map(([mint, b], i) => {
      const data = accounts.value[i]?.data;
      const multiplier = multiplierOf(data && "parsed" in data ? data : undefined, now);
      return [mint, (Number(b.raw) / 10 ** b.decimals) * multiplier];
    }),
  );
}
...
export async function mainnetHoldings(owner: PublicKey): Promise<Holding[]> {
  const balances = await token2022Balances(owner, new Set(KNOWN.keys()));
  return [...balances].map(([mint, shares]) => ({ ...KNOWN.get(mint)!, shares }));
}
